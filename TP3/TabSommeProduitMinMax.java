import java.lang.Math;
public class TabSommeProduitMinMax {
    public static void main(String[] args){
        int [] tabInt = new int[10];
        int somme = 0;
        int produit = 1;
        int min = 101;
        int max = 0;
        int nbPaire = 0;
        int n = 0;
        int b = 0;

        for (int i = 0; i < 10; i++){
            int aleatoire = (int)  (Math.random() * (100 + 1));
            tabInt[i] = aleatoire;
            somme = tabInt[i] + somme;
            produit = tabInt[i] * produit;
            if (tabInt[i] < min){
                min = tabInt[i];
            }
            if (tabInt[i] > max){
                max = tabInt[i];
            }
            int result = tabInt[i] % 2;

            if (result == 0){
                nbPaire++;
            }
        }
        int [] pair = new int[nbPaire];
        int [] impair = new int[10-nbPaire];

        for (int j = 0; j < 10; j++){
            int resultats = tabInt[j] % 2;
            if (resultats == 0){
                pair[n] = tabInt[j];
                n++;
            }
            else{
                impair[b] = tabInt[j];
                b++;
            }
        }
        for (int valeur : tabInt) {
            System.out.println(valeur);
        }
        System.out.print("Nombre pair : ");
        for (int valeurs : pair) {
            System.out.print(valeurs+"; ");
        }
        System.out.println("");
        System.out.print("Nombre impair : ");
        for (int valeurr : impair) {
            System.out.print(valeurr+"; ");
        }
        System.out.println("");
        System.out.println("La somme : "+somme);
        System.out.println("Le produit : "+produit);
        System.out.println("Le Min : "+ min);
        System.out.println("La Max : "+ max);
    }
}
